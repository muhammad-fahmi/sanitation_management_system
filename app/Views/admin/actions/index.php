...
<div class="container p-3">
            ...
            <table class="table table-striped">
                <tbody>
                    <tr>
                        <td>Lokasi</td>
                        <td>:</td>
                        <td class="text-capitalize"><?= esc($location_name) ?></td>
                    </tr>
                    <tr>
                        <td>Item</td>
                        <td>:</td>
                        <td class="text-capitalize"><?= esc($item_name) ?></td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h5>Manajemen Aksi</h5>
            <button class="btn btn-primary ms-3 d-flex align-items-center" onclick="modal('<?= $item_id ?>','add')">
                <iconify-icon icon="fa7-solid:plus" width="20" height="20"></iconify-icon>
                Tambah
            </button>
        </div>
        <div class="card-body p-3 table_container">
            <table class="table table-striped" id="table_data_action">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Nama Aksi</th>
...
<?= $this->section('script') ?>
<script>
    let table_data_action = new DataTable("#table_data_action", {
        responsive: true,
        processing: true,
        serverSide: true,
        order: [
        ...
        ajax: {
            url: '<?= site_url('admin/manage/task/get_datatable/action'); ?>',
            type: 'POST',
            data: {
                item_id: '<?= $item_id ?>'
            }
    ...
    function modal(id, type) {
        let url = "<?= base_url('admin/manage/task/modal/action') ?>";
        $.ajax({
            type: "POST",
            url: url,
            data: {
