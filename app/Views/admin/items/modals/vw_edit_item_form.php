<form id="form_edit_item" action="<?= site_url('admin/manage/task/edit/item') ?>" method="post">
    ...
    <input type="hidden" name="id" value="<?= esc(item_id($item)); ?>">
    <input type="hidden" name="location_id" value="<?= esc($item['location_id'] ?? $item['room_id'] ?? '') ?>">

    <div class="mb-3">
        <label for="edit_item" class="form-label">Nama</label>
        <input type="text" class="form-control" id="edit_item" name="item" value="<?= esc(item_name($item)) ?>"
